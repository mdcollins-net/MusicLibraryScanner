...
public interface ITrackRepository
{
    Task<int> GetOrCreateIdAsync(int albumId, int artistId, string album, string artist, int year, string title,
        int track, int duration, DateTime? dateTagged, string musicBrainzTrackId, string musicBrainzReleaseId, string musicBrainzArtistId);
}
...
public class TrackRepository(PostgresConnectionFactory connFactory) : ITrackRepository
{
    public async Task<int> GetOrCreateIdAsync(int albumId, int artistId, string album, string artist, int year,
        string title, int track, int duration, DateTime? dateTagged, string musicBrainzTrackId, string musicBrainzReleaseId, string musicBrainzArtistId)
    {
        using var conn = connFactory.CreateConnection();
        const string selectSql = "SELECT ID FROM Tracks WHERE AlbumId = @AlbumId AND ArtistId = @ArtistId LIMIT 1";
        var existing = await conn.QueryFirstOrDefaultAsync<int?>(selectSql,
            new { AlbumId = albumId, ArtistId = artistId });
        if (existing.HasValue) return existing.Value;

        const string insertSql = @"
                INSERT INTO Tracks (Album, AlbumId, Artist, ArtistId, Year, Title, Track, Duration, DateTagged, 
                                    MusicBrainzTrackId, MusicBrainzReleaseId, MusicBrainzArtistId)
                VALUES (@Album, @AlbumId, @Artist, @ArtistId, @Year, @Title, @Track, @Duration, @DateTagged, 
                        @MusicBrainzTrackId, @MusicBrainzReleaseId, @MusicBrainzArtistId)
                RETURNING ID";
        return await conn.ExecuteScalarAsync<int>(insertSql,
            new
            {
                ...
                Track = track, Duration = duration, DateTagged = dateTagged, MusicBrainzTrackId = musicBrainzTrackId,
