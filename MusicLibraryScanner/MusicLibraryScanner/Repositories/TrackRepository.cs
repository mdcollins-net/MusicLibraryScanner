...
public interface ITrackRepository {
    Task<int> GetOrCreateIdAsync(int albumId, int artistId, string album, string artist, int year, string title,
        int trackNumber, int duration, DateTime? dateTagged, string musicBrainzTrackId, string musicBrainzReleaseId,
...
public class TrackRepository(PostgresConnectionFactory connFactory) : ITrackRepository {
    public async Task<int> GetOrCreateIdAsync(int albumId, int artistId, string album, string artist, int year,
        string title, int trackNumber, int duration, DateTime? dateTagged, string musicBrainzTrackId,
        string musicBrainzReleaseId, string musicBrainzArtistId) {
        using var conn = connFactory.CreateConnection();
        const string selectSql =
            "SELECT ID FROM Tracks WHERE Album = @Album AND Artist = @Artist AND Year = @Year AND Title = @Title AND TrackNumber = @TrackNumber LIMIT 1";
        ...
        return await conn.ExecuteScalarAsync<int>(insertSql,
            new {
                ...
                TrackNumber = trackNumber, Duration = duration, DateTagged = dateTagged,
